...
deploy: handlers
	...
	sam deploy --capabilities CAPABILITY_NAMED_IAM  --template-file out.yml --stack-name $(APP) --parameter-overrides SlackToken=$(SLACK_TOKEN)
...
dev-sam:
	sam local start-api -p 3001 --parameter-overrides SlackToken=$(SLACK_TOKEN)
dev-watch:
